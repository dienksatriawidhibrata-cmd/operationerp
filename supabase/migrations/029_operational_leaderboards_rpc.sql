-- ============================================================
-- National operational leaderboards RPC
-- Migration 029
-- ============================================================
...
BEGIN
  ...
  v_expected_checklist_days := v_elapsed_days * 3;
  v_expected_preparation_days := v_elapsed_days * 3;
    ...
    SELECT
      ...
      COUNT(*) FILTER (
        WHERE COALESCE(p.updated_at, p.created_at) <= (
          CASE p.shift
            WHEN 'pagi' THEN (p.tanggal::timestamp AT TIME ZONE 'UTC') + INTERVAL '1 hour'
            WHEN 'middle' THEN (p.tanggal::timestamp AT TIME ZONE 'UTC') + INTERVAL '7 hour'
            WHEN 'malam' THEN (p.tanggal::timestamp AT TIME ZONE 'UTC') + INTERVAL '20 hour'
            ELSE NULL
          END
        )
