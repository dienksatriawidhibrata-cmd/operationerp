...
export default function DMDashboard() {
  ...
  const fetchDashboard = async () => {
    // Get branches scoped to this user's managed area
    ...
    const [ceklisRes, laporanRes, setoranRes] = await Promise.all([
    ...
    const totalSetoran = enriched.filter(b => b.setoran?.status === 'submitted').length
    const ceklisOK     = enriched.filter(b => b.ceklisPagi).length
    const laporanOK    = enriched.filter(b => b.laporan).length

    setSummary({ total: branches.length, ceklisOK, laporanOK, totalSetoran })
    setStores(enriched)
    setLoading(false)
  }
  ...
  const roleName  = profile?.role === 'district_manager' ? `DM ${(profile.managed_districts||[]).join(',')}` :
                    profile?.role === 'area_manager'     ? `AM ${(profile.managed_areas||[]).join(',')}` :
                    'Ops Manager'
  ...
  return (
    <div className="page-shell">
      ...
      <div className="flex-1 overflow-y-auto pb-24 px-4 -mt-2">
        {loading ? (
          <div className="flex justify-center py-12"><div className="animate-spin w-8 h-8 border-2 border-primary-600 border-t-transparent rounded-full" /></div>
        ) : (
          <>
            {/* Summary stats */}
            ...
            {/* Store list */}
            <p className="section-title">Status Toko — {today}</p>
